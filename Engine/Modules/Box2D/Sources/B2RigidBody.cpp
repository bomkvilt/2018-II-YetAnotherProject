...
void RigidBody::SetupCollision(int bit, b2Filter& filter_r, b2Filter& filter_s)
{
	...
	switch (type)
	{
	case ECollisionMode::eCollide: filter_r.maskBits |=  (1 << bit); filter_s.maskBits &= ~(1 << bit); break;
	case ECollisionMode::eOverlap: filter_r.maskBits &= ~(1 << bit); filter_s.maskBits |=  (1 << bit); break;
	case ECollisionMode::eIgnore : filter_r.maskBits &= ~(1 << bit); filter_s.maskBits &= ~(1 << bit); break;
